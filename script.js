...
// Amikor a dokumentum betöltődött, futtassuk le a fő logikát
...
async function handleMarkerClick(location, marker) {
    ...
    if (selectedLocations.includes(locationName)) {
        ...
        if (!weatherData[locationName]) {
            ...
            weatherData[location.name] = processWeatherData(data);
            ...
            let forecastHTML = `
                <div class="location-forecast">
                    ...
                    <div class='day-forecast'>
                        ...
                        <div><strong>Szél:</strong> Max. ${data.daily[1].maxWind} km/h, <span class='highlight'>lökések akár ${data.daily[1].maxGust} km/h</span>. ${data.nextDay.windChange ? `(${data.nextDay.windChange})` : ''}</div>
                        <div>
...
// 7 napos összefoglaló készítése
...
function processWeatherData(apiData) {
  ...
  const nextDayRain = [], hourlyTime = apiData.hourly.time, tomorrowDateStr = apiData.daily.time[1];
  let morningMaxWind = 0, afternoonMaxWind = 0;
  ...
  for(let i=0; i<hourlyTime.length; i++){
      if(hourlyTime[i].startsWith(tomorrowDateStr)){
          ...
          if (hour >= 5 && hour <= 20 && apiData.hourly.precipitation[i] > 0.1) {
              nextDayRain.push({ hour, amount: apiData.hourly.precipitation[i].toFixed(1) });
          }
          if (hour >= 5 && hour < 13 && apiData.hourly.wind_speed_10m[i] > morningMaxWind) morningMaxWind = apiData.hourly.wind_speed_10m[i];
          if (hour >= 13 && hour <= 20 && apiData.hourly.wind_speed_10m[i] > afternoonMaxWind) afternoonMaxWind = apiData.hourly.wind_speed_10m[i];
      }
  }
